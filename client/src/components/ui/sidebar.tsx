    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-[var(--sidebar-width)] p-0 [&>button]:hidden isolate border-r-0 shadow-2xl shadow-black/40"
          style={
            {
              "--sidebar-width": SIDEBAR_WIDTH_MOBILE,
              // iOS standalone PWA can render a portaled fixed dialog panel
              // transparent despite an opaque bg class, so the page behind bleeds
              // through the drawer. Force an explicit opaque paint inline and a
              // dedicated compositing layer to prevent it.
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-14 w-14 [&_svg]:size-10", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
